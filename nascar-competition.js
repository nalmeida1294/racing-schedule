...
const NascarCompetition=(()=>{
 ...
 function standings(rows,series,profiles) {
   ...
   return table(['Pos','Driver','Points','Behind','Wins','Top 5','Top 10','Starts'],sorted.map(r=>`<tr><td>${value(r.Position)}</td><td>${identity(r,series,profiles,false)}</td><td><strong>${value(r.Points)}</strong></td><td>${Number(r.Position)===1?'—':value(r['Behind Leader'])}</td>${['Wins','Top 5','Top 10','Starts'].map(k=>`<td>${value(r[k])}</td>`).join('')}</tr>`));
 }
 function results(rows,series,profiles,el) {
   ...
   el.innerHTML=`<label class="nascar-event-picker">Event<select id="nascar-result-event">${events.map(r=>`<option value="${text(r['Race ID'])}"${String(r['Race ID'])===selection[series]?' selected':''}>${text(r['Race Date'])} · ${text(r.Event)}</option>`).join('')}</select></label><h3>${text(selected.Event)}</h3><p class="f1-data-note">${text(selected['Race Date'])} · Race results</p>`+table(['Finish','Driver','Start','Laps','Led','Points','Status'],raceRows.map(r=>`<tr><td>${String(r.Disqualified)==='TRUE'?'DSQ':Number(r.Position)>0?value(r.Position):'—'}</td><td>${identity(r,series,profiles,true)}</td><td>${Number(r.Start)>0?value(r.Start):'—'}</td>${['Laps','Laps Led','Points'].map(k=>`<td>${value(r[k])}</td>`).join('')}<td>${text(r.Status)||'—'}</td></tr>`));
